...
The compiler-generated upstream uses `connectTimeout: 120` and a fixed
`toolTimeout: 630`: the final externally observable timing bucket is 600
seconds, followed by a 30-second transport allowance. This value does not shrink
with a lower configured executor timeout because execution,
...
The fixed 630-second mcpg enforcement value covers canonical bounded operation;
it is not a hard AWF wall-clock maximum. If secret-dependent processing or cleanup has already
