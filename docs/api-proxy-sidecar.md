...
optional `service-account`. AWF forwards the GitHub Actions OIDC runtime
credentials only to the api-proxy sidecar. The sidecar exchanges the JWT via
`sts.googleapis.com`, optionally impersonates the service account, and injects
the short-lived bearer token into OTLP export requests. The sidecar is trusted
and already exempt from the agent egress allowlist, so its STS calls are not
blocked.
