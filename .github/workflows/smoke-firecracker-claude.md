---
description: Smoke test Firecracker preview with Claude engine
on:
  workflow_dispatch:
  label_command:
    name: test-firecracker-claude
    events: [pull_request]
    remove_label: false
  reaction: "eyes"
permissions:
...
safe-outputs:
  ...
  add-labels:
    allowed: [smoke-firecracker-claude]
  messages:
    footer: "> 🔥🟣 *Firecracker + Claude smoke test by [{workflow_name}]({run_url})*"
    run-started: "🔥🟣 [{workflow_name}]({run_url}) is testing Firecracker with Claude..."
    run-success: "🔥🟣 [{workflow_name}]({run_url}) completed. Firecracker + Claude passed. ✅"
    run-failure: "🔥🟣 [{workflow_name}]({run_url}) reports {status}. Firecracker + Claude issue detected."
timeout-minutes: 15
sandbox:
  agent:
    id: awf
...
steps:
  - name: Pre-compute smoke test data
    ...
    run: |
      ...
      printf '%s\n' 'firecracker-claude-copyback-ok' \
        > /tmp/gh-aw/agent/expected-copyback.txt
post-steps:
  - name: Validate Firecracker workspace copyback
    run: |
      test "$(cat firecracker-agent-copyback.txt)" = "firecracker-claude-copyback-ok" || {
...
`smoke-firecracker-claude` only on PASS. On workflow dispatch, call `noop`.
